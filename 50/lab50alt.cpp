...
int main()
{
	...
	while (cin >> word)
	{
		...
		{
			//cout << isNotAlphabetical(word[0]) << endl;
			alphabeticalWordCt++;
		}
	}
...
bool isConsonant(char ch)
{
	...
	if (consonant.find_first_of(ch) == string::npos) //Searches the string for the first character that matches any of the characters specified in its arguments.
		return false;							// retrun npos when the result doesn't match
	else
		return true;
}
